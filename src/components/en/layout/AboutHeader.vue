<template>
  <header class="header">
    <Navbar active="home" />
    <div class="header-intro theme-bg-primary text-white py-5">
      <div class="container">
        <div class="profile-teaser media flex-column flex-md-row">
          <img
            class="profile-image mb-3 mb-md-0 mr-md-5 ml-md-0 rounded mx-auto"
            src="@/assets/images/profile.jpg"
            alt=""
          />
          <div class="media-body text-center text-md-left">
            <div class="lead">Hi, my name is</div>
            <h2 class="mt-0 display-4 font-weight-bold">Riccardo Giomi</h2>
            <div class="bio mb-3">
              I am a full-stack developer specialized in frontend and backend development for web application. Want to
              know more? Check out my <router-link to="/en/curriculum" class="link-on-bg">resume</router-link>.
            </div>
          </div>
        </div>
      </div>
    </div>
  </header>
</template>

<script>
import Navbar from '@/components/en/layout/Navbar.vue'

export default {
  name: 'AboutHeader',
  components: { Navbar }
}
</script>
